...
// Es mejor definir $invite_allowed_roles en config.php y usar la global.
...
if (!is_logged_in() || !in_array($user_role, $allowed_roles_for_invitar)) {
    ...
    redirect('/index.php'); // <<< Redirigir a la raíz si no autorizado
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($guest_name) || empty($expiration_date_str)) {
        ...
        redirect('/invitar.php?status=error&message=' . $error_message); // <<< Usar ruta relativa a la raíz
        exit; // Asegura que el script se detenga
    }
    ...
    if ($new_invitation_data) {
        ...
        redirect('/invitar.php?status=success&code=' . $invitation_code . '&message=' . $success_message); // <<< Usar ruta relativa a la raíz
        exit; // Asegura que el script se detenga

    } else {
        // Error al crear la invitación (la función createInvitation ya loguea el error interno)
        $error_message = urlencode("Error al crear la invitación. Por favor, revisa los logs del servidor.");
        redirect('/invitar.php?status=error&message=' . $error_message); // <<< Usar ruta relativa a la raíz
        exit; // Asegura que el script se detenga
    }
    ...
    redirect('/invitar.php'); // <<< Usar ruta relativa a la raíz
    exit; // Asegura que el script se detenga
}
